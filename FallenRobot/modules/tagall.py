...
@client.on(events.NewMessage(pattern="^/luffy$(.*)"))
async def mentionall(event):
    ...
    try:
        ...
        if isinstance(
            ...
            is_admin = True
    if not is_admin:
        return await event.respond("__Lu angkatan laut, gak bisa nyuruh gw !__")
    ...
    async for usr in client.iter_participants(chat_id):
        if not chat_id in spam_chats:
            break
        usrnum += 1
        usrtxt += f"[😘 Cium Si {usr.first_name}](tg://user?id={usr.id}),\n "
        if usrnum == 5:
            if mode == "text_on_cmd":
                txt = f"{msg}\n{usrtxt}"
